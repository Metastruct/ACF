...
function MakeXCF_Rack (Owner, Pos, Angle, Id)
	if not Owner:CheckLimit("_xcf_rack") then return false end
	
	local Gun = ents.Create("XCF_Rack")
	local List = XCF.Ents
	local Classes = XCF.Classes
	...
	undo.Create("ACF Rack")
		undo.AddEntity( Gun )
		undo.SetPlayer( Owner )
	undo.Finish()
	
	Owner:AddCount("_xcf_rack", Gun)
	Owner:AddCleanup( "acfmenu", Gun )
	
	return Gun
end
list.Set( "XCFCvars", "xcf_rack" , {"id"} )
duplicator.RegisterEntityClass("xcf_rack", MakeXCF_Rack, "Pos", "Angle", "Id")
